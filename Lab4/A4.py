...
def getCheckSum(string):
    ...
    for i in reversed(range(0, len(string) + 1, -1)): # (1, len(string) + 1, 2) 
...
def getCardType(string):
    if len(string) == 13 or len(string) == 16 and string.startswith("4"):
        return "Visa"
    if len(string) == 15 and string.startswith("34") or string.startswitch("37"):
...
if isValidNumber(cardNumber):
    if getCheckSum(cardNumber) % 10 == 0:
        print(f'{getCardType(cardNumber)} , карта действительна')
    else:
        print("Карта недействительна")
else:
